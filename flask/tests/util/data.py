...
game_statement = text("INSERT INTO game (current_player, draw_pile, discard_pile, is_over) VALUES(:current, :deck, :discard, :over)")
...
class TestData:
    ...
    def create_test_games(self):
        for game in game_data:
            self._connection.execute(game_statement, {
                'current': game['current'],
                'deck': json.dumps(game['deck'], separators=(',', ':')),
                'discard': json.dumps(game['discard'], separators=(',', ':')),
                'over': game['over']
            })